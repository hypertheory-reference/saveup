...
createSelector(selectChildJobsEntityArray, selectJobsEntityArray, (assignments, jobs) => {
  if (!id) {
    return undefined;
  } else {
    ...
    return jobs.map((j) => j.id).filter((jobId) => { 
      return assignments.filter((a) => a.jobId === jobId && a.childId === id).length === 0;
    }).map((jobId) => jobs.find((j) => j.id === jobId)) as models.JobListModel[];
  }
});
